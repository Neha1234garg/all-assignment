...
EzoicPixel.prototype = {
    ...
    AddImpPixelById: function(impId, pixelData) {
        var domainId = "0";
        if (typeof _ezaq !== "undefined" && _ezaq.hasOwnProperty("domain_id")) {
            domainId = _ezaq.domain_id.toString();
        }
        var vals = impId.split('/');
        if (this.isDefined(impId) && vals.length === 3 && this.validateData(pixelData)) {
            var adUnit = vals[0];
            pixelData = this.util.convertDataToStr(pixelData);
            this.pixels.push({type: "impression", impression_id: impId, domain_id: domainId, unit: adUnit, t_epoch: this.tEpoch, data: pixelData});
        }
    },
